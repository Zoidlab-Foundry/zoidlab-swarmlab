"""SwarmLab ⇄ Foundry integration — SpendGuard usage emission + TrustGate preflight.

Cross-app calls carry the caller's zb_session (forwarded on interactive requests) or a
minted owner session. Best-effort; never breaks a swarm run. §6.3 usage events + §6.4
policy decisions.
"""
import os
import time
...
async def emit_spend(usage, resource_id=None, feature="", correlation_id=None, environment="production"):
    """One SpendGuard usage event for a swarm run (real model + token split)."""
    ...
    ev = {"model": usage["model"], "prompt_tokens": pt, "completion_tokens": ct,
          "app": "swarmlab", "feature": feature or "swarm_run", "source": "swarmlab",
          "environment": environment, "correlation_id": correlation_id,
          "resource_ref": {"package_id": "swarm", "resource_id": resource_id, "resource_type": "swarm_run"}}
    try:
